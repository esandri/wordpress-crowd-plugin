...
Version: 0.1
Author: Emanuele Sandri & Mauro Braggio
...
function crowd_can_authenticate($username, $password) {
        $response = \Httpful\Request::post(get_option('crowd_url') . '/rest/usermanagement/1/authentication?username='.$username)
                ...
                ->addHeader("User-Agent", "Httpful/0.2.19 (cURL/7.22.0 PHP/5.3.10-1ubuntu3.25 (Linux))")
        ...
        if ($response->code == 200) {
                ...
                error_log( print_r( $response->raw_body . " - " . $response->code, true ) );
                return false;
        }
}
